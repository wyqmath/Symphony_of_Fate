...
# EsmConfig is the key to loading the model architecture without weights
...
# ==============================================================================
# ======================== MODIFICATION FOR "FROM SCRATCH" =======================
# ==============================================================================
# --- Hyperparameters for "From Scratch" Training ---
LEARNING_RATE = 1e-4             # Increased learning rate for training from scratch
BATCH_SIZE = 16
NUM_EPOCHS = 100                  # Increased epochs for convergence
WEIGHT_DECAY = 0.01
EARLY_STOPPING_PATIENCE = 5      # Increased patience

DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
EXPERIMENT_NAME = f"ESM2_From_Scratch_{MODEL_NAME.split('/')[-1]}" # Updated name
# ==============================================================================
# ==============================================================================
...
# ==============================================================================
# =========== MODIFICATION: MODEL INITIALIZATION IN ESMClassifier ==============
# ==============================================================================
class ESMClassifier(nn.Module):
    """ESM-2 based classifier. Takes a base esm model as input."""
    def __init__(self, esm_base_model, num_classes): # Modified to accept a model instance
        super(ESMClassifier, self).__init__()
        self.esm = esm_base_model
...
if __name__ == "__main__":
    ...
    if X_train_full_enc is None:
        ...
        print("\nSplit full training data into training (80%) and validation (20%) sets.")
        ...
        # ==============================================================================
        # ==================== KEY MODIFICATION FOR "FROM SCRATCH" =====================
        # ==============================================================================
        print(f"\nInitializing ESM model ({MODEL_NAME}) from SCRATCH (random weights).")
        # 1. Load the configuration of the model architecture.
        config = EsmConfig.from_pretrained(MODEL_NAME)
        
        # 2. Create a new model instance using ONLY the configuration.
        #    This ensures the model has the correct architecture but with random weights.
        esm_base = EsmModel(config)
        
        # 3. Wrap the randomly initialized base model with our classifier head.
        model = ESMClassifier(esm_base_model=esm_base, num_classes=NUM_CLASSES).to(DEVICE)
        # ==============================================================================
        # ==============================================================================
        ...
        try:
            ...
            dummy_input_ids = dummy_batch['input_ids'][0:1].to(DEVICE)
            ...
            print(f"Could not calculate GFLOPs. Error: {e}")